...
import { Hexagon, Mail, Lock, ArrowRight, AlertCircle, Loader2, X, Shield } from 'lucide-react';
...
export default function LoginModal({ isOpen, onClose }) {
    ...
    async function handleAdminSubmit(e) {
        ...
        try {
            // Check Firestore for the access code
            const docRef = doc(db, "settings", "security");
            const docSnap = await getDoc(docRef);

            if (docSnap.exists()) {
                const serverCode = docSnap.data().entryCode;
                if (adminCode === serverCode) {
                    setIsAdminVerified(true);
                } else {
                    setAdminError('Invalid access code.');
                }
            } else {
                setAdminError('System configuration error: Security settings missing.');
            }
        } catch (err) {
            console.error("Verification error:", err);
            setAdminError('Verification failed. Check network or permissions.');
        } finally {
            setLoading(false);
        }
    }
        ...
        <div className="modal-overlay" onClick={handleOverlayClick}>
            <div className="login-content">
                    ...
                    {!isAdminVerified ? (
                        <div className="admin-verify-section">
                            <div className="login-header" style={{ marginBottom: '1.5rem' }}>
                                <div className="brand-logo-container" style={{ color: 'var(--amber-400)', borderColor: 'var(--amber-900)' }}>
                                    <Shield size={32} strokeWidth={1.5} />
                                </div>
                                <h2 className="login-title" style={{ fontSize: '1.5rem' }}>Admin Access</h2>
                                <p className="login-subtitle">Enter admin code to proceed.</p>
                            </div>
                            ...
                            <form onSubmit={handleAdminSubmit}>
                                ...
                                <button type="submit" className="btn-login" style={{ background: 'linear-gradient(to right, var(--amber-600), var(--amber-500))' }}>
                                    Verify
                                    <ArrowRight size={16} />
                                </button>
                            </form>
                ...
                <div className="login-footer">
                    <p>&copy; {new Date().getFullYear()} Bora Karataş. All rights reserved.</p>
                </div>
            </div>
        </div>
    );
