...
export default function App() {
  ...
  const downloadPDF = () => {
    ...
    doc.text(`${activity.meta.level} | ${activity.meta.type.toUpperCase()}`, margin, y);
    y += 15;

    // Questions (Simplified Loop for brevity)
    doc.setFontSize(12); doc.setTextColor(0);
    doc.text("Instructions: " + activity.student_worksheet.instructions, margin, y, { maxWidth: 170 });
    y += 20;
    ...
    activity.student_worksheet.questions.forEach((q, i) => {
      ...
      if (q.options && q.options.length > 0) {
        q.options.forEach(opt => { doc.text(`[ ] ${opt}`, margin + 10, y); y += 6; });
      } else if (activityType === 'true_false') {
        doc.text("[ TRUE ]   [ FALSE ]", margin + 10, y); y += 10;
      } else {
        doc.line(margin, y + 5, 180, y + 5); y += 12;
      }
      y += 5;
    });
